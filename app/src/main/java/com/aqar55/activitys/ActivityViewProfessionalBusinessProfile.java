...
public class ActivityViewProfessionalBusinessProfile extends AppCompatActivity implements OnMapReadyCallback {
    ...
    public static Intent getIntent(Context context, GetUserDetails.DataEntity dataEntity) {
        Intent intent = new Intent(context, ActivityViewProfessionalBusinessProfile.class);
    ...
    private void getIntentdata() {
        if (getIntent() != null) {
            if ((GetUserDetails.DataEntity) getIntent().getSerializableExtra("Data") != null) {
                dataAll = (GetUserDetails.DataEntity) getIntent().getSerializableExtra("Data");
                type = dataAll.getType();
                userId = dataAll.getCommanUserProfessionalId();
                professionalUserId = dataAll.getUserId();
                ...
                professionalUserId = dataProperty.getProfessionalUserId();
        ...
        if (type.equalsIgnoreCase("sale") || type.equalsIgnoreCase("rent")) {
            ...
            getUserDetailsCall.enqueue(new Callback<GetUserDetails>() {
                ...
                public void onResponse(Call<GetUserDetails> call, Response<GetUserDetails> response) {
                    MyDialog.getInstance(ActivityViewProfessionalBusinessProfile.this).hideDialog();
                    if (response.isSuccessful()) {
                        if (response.body().getResponseCode() == 200) {
                            ...
                            Toast.makeText(ActivityViewProfessionalBusinessProfile.this, response.body().getResponseMessage(), Toast.LENGTH_SHORT).show();
                        }
                    }
                }
            ...
            if (type.equalsIgnoreCase("professional")) {
                ...
                Call<GetUserDetails> getUserDetailsCall;
                if (ModelManager.modelManager().getCurrentUser() != null)
                    getUserDetailsCall = api.getBusinessOrProfessionalDetails(dataAll.getUserId(), dataAll.getCommanUserBusinessId(), "business");
                else
                    getUserDetailsCall = api.getBusinessOrProfessionalDetails(dataAll.getUserId(), userId, dataAll.getType());
                ...
                userId = dataAll.getCommanUserProfessionalId();
                if (ModelManager.modelManager().getCurrentUser() != null)
                    getUserDetailsCall = api.getBusinessOrProfessionalDetails(dataAll.getUserId(), userId, "professional");
                else
                    getUserDetailsCall = api.getBusinessOrProfessionalDetails(dataAll.getUserId(), userId, dataAll.getType());
    ...
    private void setBusinessData(GetUserDetails.DataEntity dataEntity) {
        ...
        tvLicenseNumber2.setText("License No. : " + "000000000000");
        tvDurationTime.setText("Member since " + dataEntity.getMembersince());
        tvUpdatedTime.setText("Updated : " + dataEntity.getCreatedat());
        tvSpecialities1.setText(dataEntity.getSpecialities());
        tvViewedTime.setText("Viewed " + dataEntity.getCount() + " times");
